...
class SendDailyReportToTelegram extends Command
{
    ...
    public function handle()
    {
        ...
        $chatId = env('TELEGRAM_CHAT_ID'); // -5193251186
        ...
        $orders = Order::with(['items', 'receipts'])->get();
        $debtsToUsSyp = 0;
        $debtsToUsUsd = 0;
        ...
        foreach ($orders as $order) {
            ...
            $debtsToUsSyp += ($totalSyp - $paidSyp);
            $debtsToUsUsd += ($totalUsd - $paidUsd);
        }
        ...
        if ($newOrdersList->count() > 0) {
            ...
            foreach ($newOrdersList->take(5) as $order) {
                $message .= "#{$order->order_number} - {$order->customer_name}\n";
            }
        ...
        if ($oldInProgress->count() > 0) {
            ...
            foreach ($oldInProgress as $order) {
                $days = (int) Carbon::parse($order->updated_at)->diffInDays(Carbon::now());
                $message .= "#{$order->order_number} - {$order->customer_name} ({$days} يوم)\n";
            }
            $message .= "\n";
        }
