...
<form action="{{ route('receipts.update', $receipt) }}" method="POST" class="form-container">
    ...
    <div class="form-row">
        <div class="form-group">
            <label>المبلغ</label>
            <input type="number" name="amount" value="{{ old('amount', \App\Helpers\TranslationHelper::formatAmountForInput($receipt->amount)) }}" min="0" step="0.000001" placeholder="أدخل المبلغ المقبوض" required>
            @error('amount')
